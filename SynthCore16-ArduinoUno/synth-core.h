...
public:
  ...
  INLINE static void receive_midi_byte(uint8_t b) {
    if (is_data_byte(b)) {
      if (m_system_exclusive) {
        ...
        program_change(b);
      } else if(m_running_status == (CHANNEL_BYTE | MIDI_CH)) {
        channel_change(b);
      }
  ...
  INLINE static void channel_change(uint8_t channel_number) {
    IVoice<0>::channel_change(channel_number);
  }
...
private:
  ...
  INLINE static boolean is_channel_byte(uint8_t b) {
    return b >= CHANNEL_BYTE;
  }
